		...
		<section class="container">
			...
			<section class="c-sort-box">
				<section class="c-s-dl">
					<dl>
						<dt>
							<span class="c-999 fsize14">講座別</span>
						</dt>
						<dd class="c-s-dl-li">
							<ul class="clearfix">
								<li <c:if test="${queryCourse.subjectId==0}">class="current"</c:if>><a onclick="submitForm(1,0)" title="すべて" href="javascript:void(0)">すべて</a></li>
							...
							<aside class="c-s-more">
								<a href="javascript: void(0)" title="" class="fsize14 c-master">[展開]</a>
							</aside>
						</dd>
					</dl>
				...
				<div class="js-wrap">
					...
					<section class="fl">
						<ol class="js-tap clearfix">
							<li <c:if test="${queryCourse.order=='FOLLOW'}">class="current bg-orange"</c:if>><a title="ランキング" onclick="submitForm(3,'FOLLOW')" href="javascript:void(0)">ランキング</a></li>
							...
							<li <c:if test="${queryCourse.order=='ASCENDING'||queryCourse.order=='DESCENDING'}">class="current bg-orange"</c:if>><a title="価格" onclick="submitForm(4,'<c:if test="${not empty queryCourse.order}">${queryCourse.order }</c:if><c:if test="${empty queryCourse.order}">ONE</c:if>')" href="javascript:void(0)">価格<span><c:if test="${queryCourse.order=='ASCENDING' }">↑</c:if><c:if test="${queryCourse.order=='DESCENDING' }">↓</c:if></span></a></li>
						</ol>
					</section>
				</div>
				<div class="mt40">
					<c:if test="${empty courseList}">
						...
						<section class="no-data-wrap">
							<em class="icon30 no-data-ico">&nbsp;</em> <span class="c-666 fsize14 ml10 vam">関連情報が存在していない...</span>
						</section>
						<!-- /无数据提示 结束-->
					</c:if>
									...
									<li>
										<div class="cc-l-wrap">
											<section class="course-img">
												...
												<div class="cc-mask">
													<a href="${ctx}/front/couinfo/${course.courseId}" title="" class="comm-btn c-btn-1">研修を始めに</a>
												</div>
											</section>
											...
											<section class="mt10 hLh20 of">
												<c:if test="${course.currentPrice=='0.00' }">
													<span class="fr jgTag bg-green"><tt class="c-fff fsize12 f-fA">無料</tt></span>
												</c:if>
												<c:if test="${course.currentPrice!='0.00' }">
													<span class="fr jgTag bg-orange"><tt class="c-fff fsize14 f-fG">￥${course.currentPrice }</tt></span>
												</c:if>
												<span class="fl jgAttr c-ccc f-fA"> <tt class="c-999 f-fA">${course.pageBuycount }人目</tt> | <tt class="c-999 f-fA">${course.pageViewcount }閲覧</tt>
												</span>
											</section>
										</div>
									</li>
