...
<body>
	<article class="col-7 fl">
		<div class="u-r-cont">
			<section>
				<div>
					<span class="fr"><a href="${ctx }/questions/toadd" title="" class="comm-btn c-btn-6" style="font-size: 16px; height: 22px; line-height: 22px; padding: 0 20px;">お問い合わせ</a></span>
					<section class="c-infor-tabTitle c-tab-title cnew-tab-title">
						<a href="javascript: void(0)" title="My Q&A" style="cursor: default;">My Q&A</a>
						<a href="${ctx }/uc/myquestions/list" title="Wo的提问" class="current">My 質問</a>
						<a href="${ctx }/uc/myrepquestions/list" title="Wo的回答">My 答え</a>
					</section>
				</div>
				<div class="mt40">
												...
												<h3 class="hLh30 txtOf mt5">
													...
													<c:if test="${not empty question.questionsCommentList }">
														<c:if test="${question.status==0 }">
															<span class="fsize12 c-999 vam"> <tt class="c-ccc f-fM mr5">[最新答え]</tt>
														...
														<c:if test="${question.status==1 }">
															<span class="fsize12 c-999 vam"> <tt class="c-green f-fM mr5">[一番良い答え]</tt>
																	<c:forEach items="${question.questionsCommentList }" var="questionsComment">
																		<c:out value="${questionsComment.content }"></c:out>
																	</c:forEach>
															</span>
